...
def _geometry(value: str) -> str | None:
    ...
    if not match or int(match.group(1)) <= 0 or int(match.group(2)) <= 0:
        return None
    return f"{int(match.group(1))}x{int(match.group(2))}+{int(match.group(3))}+{int(match.group(4))}"
...
def _select_region() -> str | None:
    ...
    if not slurp:
        return None
    try:
        process = subprocess.run([slurp, "-f", "%wx%h+%x+%y"], capture_output=True, text=True, check=False)
    except OSError:
        return None
...
def _notify(title: str, body: str) -> None:
    ...
    if notify:
        try:
            subprocess.Popen([notify, "-a", "Clavis Shell", title, body], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except OSError:
            pass
...
def start(args) -> Result:
    ...
    if active:
        return response(state, "record.start", ok_value=False, error_value=error("recording_already_active", "A screen recording is already active"), exitCode=4)
    ...
    if args.target == "region" and geometry is None:
        return fail("record.start", 2, "selection_cancelled", "screen region selection was cancelled", cancelled=True, exitCode=2)
    ...
    process, start_error = spawn(gsr, command, Path(temporary).with_suffix(Path(temporary).suffix + ".log"))
    if process is None:
        state["state"] = "error"
...
def status(args) -> Result:
    state, active = active_state("record", "gpu-screen-recorder", load("record").get("temporaryPath", ""))
    return response(state, "record.status", ok_value=not state.get("error"), error_value=state.get("error"), exitCode=0 if not state.get("error") else 5)
...
def _finalize(state: dict) -> Result:
    ...
    try:
        ...
        save("record", state)
        return response(state, "record.stop", ok_value=False, error_value=state["error"], exitCode=POSTPROCESS_FAILURE)
    state.update({"state": "completed", "pid": 0, "processStartTicks": None, "completedAtMs": now_ms(), "temporaryPath": "", "error": None})
    save("record", state)
    if getattr(_finalize, "copy_to_clipboard", False):
        wl_copy = shutil.which("wl-copy")
        if wl_copy:
            subprocess.run([wl_copy, "--type", "text/uri-list"], input=(output.resolve().as_uri() + "\n").encode(), check=False)
...
def stop(args) -> Result:
    ...
    if state.get("state") not in {"recording", "paused", "starting"}:
        return response(state, "record.stop", ok_value=False, error_value=error("no_active_recording", "There is no active screen recording"), exitCode=5)
    if not matches(identity, "gpu-screen-recorder", state.get("temporaryPath", "")):
        state["state"] = "error"
        state["error"] = error("recorder_identity_mismatch", "refusing to stop an unverified recorder process")
        save("record", state)
        return response(state, "record.stop", ok_value=False, error_value=state["error"], exitCode=RECORDER_STOP_FAILURE)
    state["state"] = "stopping"
    save("record", state)
    stopped, forced, reason = stop_verified(identity, "gpu-screen-recorder", state.get("temporaryPath", ""))
    if not stopped:
        state["state"] = "error"
...
def toggle_pause(args) -> Result:
    state = load("record")
    identity = process_fields(state)
    if state.get("state") not in {"recording", "paused"} or not matches(identity, "gpu-screen-recorder", state.get("temporaryPath", "")):
        return response(state, f"record.{args.action}", ok_value=False, error_value=error("no_active_recording", "There is no verified screen recording"), exitCode=5)
    try:
        os.kill(identity.pid, __import__("signal").SIGUSR2)
    except OSError as exc:
        return response(state, f"record.{args.action}", ok_value=False, error_value=error("recorder_pause_failed", str(exc)), exitCode=GENERAL_FAILURE)

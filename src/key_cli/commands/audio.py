...
from ..recording.common import active_state, new_state, now_ms, output_directory, process_fields, response, spawn
from ..recording.state import StateError, load, locked, save
from ..utils.output import DEPENDENCY_FAILURE, GENERAL_FAILURE, POSTPROCESS_FAILURE, RECORDER_START_FAILURE, RECORDER_STOP_FAILURE, Result, error, fail
...
def resolve_source(source_type: str) -> tuple[dict | None, dict | None]:
    ...
    if source_type == "mic":
        ...
        if not preferred or preferred.get("name", "").endswith(".monitor") or not usable(preferred):
            preferred = next((item for item in sources if not str(item.get("name", "")).endswith(".monitor") and usable(item)), {})
        if not preferred:
        ...
        return {"type": "mic", "name": str(preferred.get("name") or ""), "nodeName": node, "description": str(preferred.get("description") or ""), "captureSink": False}, None
    ...
    if not sink or not monitor or not str(monitor.get("name", "")).endswith(".monitor") or not usable(monitor):
        return None, error("system_monitor_unavailable", "default output has no usable monitor source", defaultSink=info.get("default_sink_name"), monitorSource=monitor_name)
    return {"type": "system", "name": monitor_name, "nodeName": prop(sink, "node.name") or str(sink.get("name") or ""), "description": str(sink.get("description") or ""), "captureSink": True}, None
...
def start(args) -> Result:
    ...
    if active:
        return response(state, "audio.start", ok_value=False, error_value=error("audio_recording_already_active", "an audio recording is already active"), exitCode=4)
    ...
    if missing:
        return fail("audio.start", DEPENDENCY_FAILURE, "dependency_missing", "missing: " + ", ".join(missing), dependency=missing[0], exitCode=DEPENDENCY_FAILURE)
    source, source_error = resolve_source(args.source)
    if not source:
        return response(state, "audio.start", ok_value=False, error_value=source_error, exitCode=GENERAL_FAILURE)
    ...
    try:
        ...
        return fail("audio.start", GENERAL_FAILURE, "output_directory_unwritable", str(exc), exitCode=GENERAL_FAILURE)
    ...
    session.update({"state": "starting", "source": source, "temporaryPath": str(temporary), "outputPath": str(output)})
    ffmpeg = shutil.which("ffmpeg")
    command = ["-hide_banner", "-nostdin", "-loglevel", "warning", "-f", "pulse", "-i", source["name"], "-vn", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", "-y", str(temporary)]
...
def status(args) -> Result:
    ...
    return response(state, "audio.status", ok_value=not state.get("error"), error_value=state.get("error"), exitCode=0 if not state.get("error") else 5)
...
def _validate_and_finalize(state: dict) -> Result:
    ...
    if not ffprobe:
        state["state"] = "error"
        state["error"] = error("dependency_missing", "ffprobe is not installed", dependency="ffprobe")
        save("audio", state)
        return response(state, "audio.stop", ok_value=False, error_value=state["error"], exitCode=DEPENDENCY_FAILURE)
    if not temporary.is_file() or temporary.stat().st_size == 0:
        state["state"] = "error"
        state["error"] = error("audio_output_invalid", "audio output is missing or empty")
        save("audio", state)
        return response(state, "audio.stop", ok_value=False, error_value=state["error"], exitCode=POSTPROCESS_FAILURE)
    try:
        probe = subprocess.run([ffprobe, "-v", "error", "-show_entries", "format=duration:stream=codec_type", "-of", "json", str(temporary)], capture_output=True, text=True, timeout=10, check=False)
        data = json.loads(probe.stdout or "{}")
        duration = float((data.get("format") or {}).get("duration") or 0)
        streams = data.get("streams") or []
        if probe.returncode != 0 or duration <= 0 or not any(item.get("codec_type") == "audio" for item in streams):
        ...
        save("audio", state)
        return response(state, "audio.stop", ok_value=False, error_value=state["error"], exitCode=POSTPROCESS_FAILURE)
    state.update({"state": "completed", "pid": 0, "processStartTicks": None, "completedAtMs": now_ms(), "temporaryPath": "", "error": None, "duration": duration})
    save("audio", state)
...
def stop(args) -> Result:
    ...
    if state.get("state") not in {"recording", "starting"}:
        return response(state, "audio.stop", ok_value=False, error_value=error("no_active_audio_recording", "there is no active audio recording"), exitCode=5)
    if not matches(identity, "ffmpeg", state.get("temporaryPath", "")):
        state["state"] = "error"
        state["error"] = error("audio_recorder_identity_mismatch", "refusing to stop an unverified ffmpeg process")
        save("audio", state)
        return response(state, "audio.stop", ok_value=False, error_value=state["error"], exitCode=RECORDER_STOP_FAILURE)

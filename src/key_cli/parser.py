...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="key", description="Clavis shell lifecycle and discrete task CLI")
    parser.add_argument("-v", "--version", dest="version_flag", action="store_true", help="print the key-cli version")
    commands = parser.add_subparsers(dest="command", metavar="COMMAND")

    shell_parser = commands.add_parser("shell", help="start, stop, or inspect the Clavis Quickshell")
    shell_parser.set_defaults(handler=shell)
    shell_parser.add_argument("--daemon", "-d", action="store_true", help="pass -d to qs")
    shell_parser.add_argument("--kill", "-k", action="store_true", help="stop the clavis qs configuration")
    shell_parser.add_argument("--log", "-l", action="store_true", help="print the clavis qs log")
    shell_parser.add_argument("--show", "-s", action="store_true", help="show the clavis IPC methods")
    ...
    clipboard_parser = commands.add_parser("clipboard", help="operate on the cliphist clipboard backend")
    ...
    for action in ("clear", "status", "watch", "store"):
        help_text = f"internal clipboard {action}" if action in {"watch", "store"} else f"clipboard {action}"
